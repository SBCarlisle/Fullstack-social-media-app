...
export default class RegistrationForm {
  ...
  events() {
    ...
    //on blur will run when you exit off of focus of a field
  ...
  isDifferent(el, handler) {
    if(el.previousValue != el.value){
      //this .call method is to keep the 'this' keyword scoped to the value we pass it, in this case we are passing it 'this' which refers to the overall object
      handler.call(this)
    }
  ...
  usernameImmediately() {
    ...
    if(this.username.value.length > 30) {
      this.showValidationError(this.username, "Username cannont exceed 30 characters.")
    }
  ...
  passwordImmediately() {
    if (this.password.value.length > 50) {
      this.showValidationError(this.password, "Password must be less than 50 characters")
    }
  ...
  emailAfterDelay(){
    //regex will eval true if text@text
